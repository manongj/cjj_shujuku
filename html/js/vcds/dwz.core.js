	...
	init:function(pageFrag, options){
		...
		jQuery.ajax({
			...
			success: function(xml){
				...
				$(xml).find("_TMPL_").each(function(){
					var id = $(this).attr("id");
					if (id) DWZ.tmpl[id] = $(this).text();
				});
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				...
				success: function(response){
					...
					if (json.statusCode==DWZ.statusCode.timeout){//超时
					...
					if (json.statusCode==DWZ.statusCode.error){//出错
						if (json.message) alertMsg.error(json.message);
					}else {
						if(json.statusCode==DWZ.statusCode.ok){//正确，处理json数据
							var mode = tmpl.newInstance(json.callbackType);
							var html = mode.create(DWZ.jsonEval(json.data));//对从后台传出来的JSON数据再进行包装
							$this.html(html).initUI();
						} else {//其他，处理html数据
							$this.html(response).initUI();
							/*var js = response.match(/@import\((.*)\)/);
							if(js.length!=0){
								js = js[1].split(",");
								for(var i=0;i<js.length;++i){
									var script = "<script type='text/javascript' src='"+js[i]+"'></script>";
									$this.append(script);
								}
							}*/
						}
