(function($){
	...
	$.fn.jBar = function(options){
		...
		return this.each(function(){
			var jbar = this;
			var sbar = $(op.sideBar2, jbar);//左侧边栏面板，初始时隐藏
			var bar = $(op.sideBar, jbar);//菜单主体面板
			$(op.toggleBut, bar).click(function(){//菜单主体面板上的收缩按钮《
				DWZ.ui.sbar = false;
				$(op.splitBar).hide();
				...
				$(op.collapse,sbar).click(function(){//左侧边栏面板
					var sbarwidth = sbar.cssv("left") + sbar.outerWidth();
					if(bar.is(":hidden")) {
			...
			$(op.toggleBut, sbar).click(function(){//左侧边栏上的弹出按钮》
				DWZ.ui.sbar = true;
